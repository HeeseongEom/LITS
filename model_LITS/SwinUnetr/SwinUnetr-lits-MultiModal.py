...
#text_feature = torch.ones(1,14,1024).to(device)
file_path = "C:\\Users\\DESKTOP\\Documents\\연구\\MultiModal\\feature_map\\hidden_states.npy"
text_feature_np = np.load(file_path)
#text_feature = torch.from_numpy(text_feature_np).to(device)
text_feature = torch.ones(1,14,1024).to(device)
print(text_feature.size(2))
model = SwinUNETR(
    img_size=(128, 128, 64),
    in_channels=1,
    out_channels=3,
    feature_size=24,
    text_feature_dim=(text_feature.size(1),text_feature.size(2))
).to(device)
...
def train(global_step, train_loader, dice_val_best, global_step_best, last_fc_size, text_feature=None):
    model.train()
    ...
    for step, batch in enumerate(epoch_iterator):
        ...
        with torch.cuda.amp.autocast():
            logit_map = model(x, text_feature)
...
while global_step < max_iterations:
    global_step, dice_val_best, global_step_best = train(global_step, train_loader, dice_val_best, global_step_best, last_fc_size, text_feature)
...
with torch.no_grad():
    for num in range(case_num):
        ...
        val_outputs = sliding_window_inference(val_inputs, (64, 128, 128), 4, model, overlap=0.8)
...
with torch.no_grad():
    for num in range(6):
        ...
        out = sliding_window_inference(img, (64, 128, 128), 4, model, overlap=0.8)
