...
root_dir = 'D:\severance\SwinUnetr'
print(root_dir)
    ...
    with torch.no_grad():
        for batch in epoch_iterator_val:
            ...
            with torch.cuda.amp.autocast():
                val_outputs = sliding_window_inference(val_inputs, (192, 192, 64), 2, model)
...
def test(epoch_iterator_test, last_fc_size):
    ...
    with torch.no_grad():
        ...
        for batch in epoch_iterator_test:
            ...
            with torch.cuda.amp.autocast():
                test_outputs = sliding_window_inference(test_inputs, (192, 192, 64), 2, model)
...
def train(global_step, train_loader, dice_val_best, global_step_best, last_fc_size):
    ...
    for step, batch in enumerate(epoch_iterator):
        ...
        if (global_step % eval_num == 0 and global_step != 0) or global_step == max_iterations:
            ...
            dice_val = validation(epoch_iterator_val, last_fc_size)
            epoch_loss /= step
        ...
        if (global_step % (eval_num) == 0 and global_step != 0) or global_step == max_iterations:
            test_metric_values.append(test(test_loader, last_fc_size))
...
def calculate_asd_rvd(model, data_loader):
    model.eval()
    asd_values = []
    rvd_values = []
    ...
    with torch.no_grad():
        for batch in data_loader:
            ...
            if targets.max() >= 3:
                print("\nReplace labels >= {} to 0".format(3))
                targets[targets >= 3] = 0

            outputs = sliding_window_inference(inputs, (96, 96, 96), 4, model)

            # 예측과 레이블을 원-핫 인코딩으로 변환 후 클래스별로 처리
            predictions = torch.nn.functional.one_hot(torch.argmax(outputs, dim=1), num_classes=3).permute(0, 4, 1, 2, 3)
            
            labels_one_hot = torch.nn.functional.one_hot(targets.long(), num_classes=3)
            labels_one_hot = torch.squeeze(labels_one_hot, 1).permute(0, 4, 1, 2, 3)
            for i in range(3):  # 클래스 0은 배경이므로 계산에서 제외
                pred_class = predictions[:, i:i+1].cpu()  # 클래스 i에 대한 예측
                label_class = labels_one_hot[:, i:i+1].cpu()  # 클래스 i에 대한 레이블

                asd = compute_average_surface_distance(pred_class.cpu().numpy(), label_class.cpu().numpy()).cpu().numpy()
                #asd_mean = np.mean([np.mean(asd[0]), np.mean(asd[1])])
                
                asd_values.append(asd[0][0])
                
                # RVD 계산
                rvd = np.abs(pred_class.sum() - label_class.sum()) / float(label_class.sum())
                #print(rvd)
                #rvd_mean = np.mean([np.mean(rvd[0]), np.mean(rvd[1])])
                rvd_values.append(rvd.cpu().numpy())
                #print(rvd_values)
        asd_mean = np.mean(asd_values)
        rvd_mean = np.mean(rvd_values)
    return asd_mean, rvd_mean

# ASD와 RVD 계산

model.load_state_dict(torch.load(os.path.join(root_dir, "best_metric_model.pth")))
data_loader = test_loader  # 또는 test_loader

# ASD와 RVD 계산
asd, rvd = calculate_asd_rvd(model, data_loader)
print(f"Average ASD: {asd}, Average RVD: {rvd}")
...
plt.plot(x, test_metric_values, label='Test Metric')
plt.xlabel("Iteration")
plt.legend()
plt.show()
###-------------------------------------------------Check best model-------------------------------------------
slice_map = {"CT393.nii.gz":17, "CT394.nii.gz":15, "CT395.nii.gz":30}
case_num = 3
model.load_state_dict(torch.load(os.path.join(root_dir, "best_metric_model.pth")))
model.eval()
with torch.no_grad():
    for num in range(case_num):
        img_name = os.path.split(val_ds[num]["image"].meta["filename_or_obj"])[1]
        img = val_ds[num]["image"]
        label = val_ds[num]["label"]
        val_inputs = torch.unsqueeze(img, 1).cuda()
        val_labels = torch.unsqueeze(label, 1).cuda()
        val_outputs = sliding_window_inference(val_inputs, (192, 192, 64), 4, model, overlap=0.8)
...
###=============================================Inference==============================
test_slice_map = {"CT413.nii.gz": 31, "CT414.nii.gz": 22, "CT415.nii.gz": 42}  # 예시 슬라이스 맵 (실제 파일명에 맞게 수정 필요)
test_case_num = 3  # 테스트 케이스의 수

model.load_state_dict(torch.load(os.path.join(root_dir, "best_metric_model.pth")))
model.eval()
with torch.no_grad():
    for num in range(test_case_num):
        img_name = os.path.split(test_ds[num]["image"].meta["filename_or_obj"])[1]
        ...
        test_outputs = sliding_window_inference(test_inputs, (192, 192, 64), 4, model, overlap=0.8)
        
        plt.figure("check_test", (18, 6))
        plt.subplot(1, 3, 1)
        plt.title("Test Image")
        plt.imshow(test_inputs.cpu().numpy()[0, 0, :, :, test_slice_map.get(img_name, 50)], cmap="gray")
        plt.subplot(1, 3, 2)
        plt.title("Test Label")
        plt.imshow(test_labels.cpu().numpy()[0, 0, :, :, test_slice_map.get(img_name, 50)])
        plt.subplot(1, 3, 3)
        plt.title("Test Output")
        plt.imshow(torch.argmax(test_outputs, dim=1).detach().cpu()[0, :, :, test_slice_map.get(img_name, 50)])
        plt.show()