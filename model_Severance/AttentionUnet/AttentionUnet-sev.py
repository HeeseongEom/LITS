...
root_dir = 'D:\\severance\\models\\AttentionUnet'
print(root_dir)
    ...
    with torch.no_grad():
        for batch in epoch_iterator_val:
            ...
            with torch.cuda.amp.autocast():
                val_outputs = sliding_window_inference(val_inputs, (224, 224, 64), 2, model)
...
def test(epoch_iterator_test, last_fc_size):
    ...
    with torch.no_grad():
        ...
        for batch in epoch_iterator_test:
            ...
            with torch.cuda.amp.autocast():
                test_outputs = sliding_window_inference(test_inputs, (224, 224, 64), 2, model)
...
'''Training (10046 / 10000 Steps) (loss=0.13613): 100%|██████| 197/197 [13:50<00:00,  4.21s/it] 
epoch_loss_values: [1.2333795428276062, 1.1150051355361938, 1.0137338280677795, 0.8919385992563688, 0.7941821217536926, 0.7688856093507064, 0.6814716322855516, 0.6559052753448487, 0.5849159732460976, 0.5567089819139049, 0.523838981109507, 0.49325201640257965, 0.4571990355849266, 0.4346365284088046, 0.4116987521233766, 0.39926144906452726, 0.4135493326645631, 0.374683875116435, 0.36480716281923753, 0.37174732592262205, 0.34908767766319215, 0.3469439891736899, 0.319307063307081, 0.32219793759796717, 0.3129714135276644, 0.31549611442451236, 0.2789270345394204, 0.3057235698489582, 0.3053685045716437, 0.28201583682835757, 0.28794642743912147, 0.2889779723182167, 0.2813880269229412, 0.274498381371637, 0.26294177348883646, 0.27341097565965916, 0.28928430238738656, 0.27670274234336356, 0.2574555944588225, 0.2544725375973489, 0.26117958933595686, 0.2553168889575117, 0.24797816815284582, 0.25209972060712654, 0.24301517442526185, 0.25814310225437015, 0.24766761930265896, 0.2464830824527247, 0.2466699865118072, 0.24345236059450945] metric_values: [0.4522830843925476, 0.5159690976142883, 0.5559478998184204, 0.6328349113464355, 0.6238735914230347, 0.6579896211624146, 0.6217387914657593, 0.6889113187789917, 0.6741029620170593, 0.6755625009536743, 0.6895250082015991, 0.6839709281921387, 0.6917687058448792, 0.7160797715187073, 0.6763015389442444, 0.6600688099861145, 0.7406942248344421, 0.734353244304657, 0.7190002202987671, 0.6805328726768494, 0.7327792048454285, 0.7383044958114623573, 0.7325924634933472, 0.7490535974502563, 0.754804253578186, 0.7559308409690857, 0.7179809212684631, 0.7382980585098267, 0.7524421811103821, 0.7458829879760742, 0.747190535068512, 0.7460317611694336, 0.7597485780715942, 0.7598245739936829, 0.7360482215881348, 0.7677224278450012, 0.7518669366836548, 0.7493702173233032, 0.7651580572128296, 0.7610781788825989, 0.7610544562339783, 0.7475293278694153, 0.760762095451355, 0.760948896408081, 0.7547045946121216] test_metric_values: [0.5210065841674805, 0.6933587789535522, 0.6742469072341919, 0.6749361157417297, 0.6265227794647217, 0.6969871520996094, 0.7129222750663757, 0.7338629961013794, 0.7233417630195618, 0.7374571561813354, 0.7346518635749817, 0.7428944706916809, 0.7815086245536804, 0.7734570503234863, 0.7365714907646179, 0.7505026459693909, 0.7580179572105408]'''

# Convert to JSON string
json_data = json.dumps(data, indent=4)

# Write to file
with open('D:\\severance\\models\\AttentionUnet\\metrics.json', 'w') as file:
    file.write(json_data)

###------------------------------------Plot Loss, metric------------------------------------------------

'''
with open('D:\\severance\\AttentionUnet\\metrics.json', 'r') as file:
    json_data = file.read()
...
plt.show()
loss_graph = "D:\\severance\\models\\AttentionUnet\\loss_graph"
...
with torch.no_grad():
    for num in range(20):
        for ran in range(2):
            ...
            print(img.shape)
            out = sliding_window_inference(img, (224, 224, 64), 4, model, overlap=0.8)
            print(out.shape)
            ...
            label = val_ds[num][ran]["label"].squeeze().cpu().numpy().astype(np.uint8)
            label = nib.Nifti1Image(label, affine=np.eye(4))

            nib.save(img,f"D:\\severance\\models\\AttentionUnet\\val_result\\{num}-{ran}-output.nii.gz")
            nib.save(label,f"D:\\severance\\models\\AttentionUnet\\val_result\\{num}-{ran}-label.nii.gz")
...
with torch.no_grad():
    for num in range(19):
        for ran in range(2):
            ...
            nib.save(img,f"D:\\severance\\models\\AttentionUnet\\test_result\\{num}-{ran}-output.nii.gz")
            nib.save(label,f"D:\\severance\\models\\AttentionUnet\\test_result\\{num}-{ran}-label.nii.gz")
###-------------------------------------------------Check best model-------------------------------------------
...
with torch.no_grad():
    for num in range(case_num):
        for ran in range(2):
            img = val_ds[num][ran]["image"]
            label = val_ds[num][ran]["label"]
            val_inputs = torch.unsqueeze(img, 1).cuda()
            val_labels = torch.unsqueeze(label, 1).cuda()
            val_outputs = sliding_window_inference(val_inputs, (224, 224, 64), 4, model, overlap=0.8)
...
with torch.no_grad():
    for num in range(test_case_num):
        for ran in range(2):
            img = test_ds[num][ran]["image"]
            label = test_ds[num][ran]["label"]
            test_inputs = torch.unsqueeze(img, 1).cuda()
            test_labels = torch.unsqueeze(label, 1).cuda()
            test_outputs = sliding_window_inference(test_inputs, (224, 224, 64), 4, model, overlap=0.8)
            
            plt.figure("check_test", (18, 6))
            plt.subplot(1, 3, 1)
            plt.title("Test Image")
            plt.imshow(test_inputs.cpu().numpy()[0, 0, :, :, slice_map[40]], cmap="gray")
            plt.subplot(1, 3, 2)
            plt.title("Test Label")
            plt.imshow(test_labels.cpu().numpy()[0, 0, :, :, slice_map[40]])
            plt.subplot(1, 3, 3)
            plt.title("Test Output")
            plt.imshow(torch.argmax(test_outputs, dim=1).detach().cpu()[0, :, :, slice_map[40]])
            plt.show()