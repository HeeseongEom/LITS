...
def test(epoch_iterator_test, last_fc_size):
    ...
    with torch.no_grad():
        ...
        for batch in epoch_iterator_test:
            ...
            with torch.cuda.amp.autocast():
                test_outputs = sliding_window_inference(test_inputs, (128, 128, 64), 2, model)
...
# Write to file
with open('D:\\severance\\models\\UNetr\\metrics.json', 'w') as file:
    file.write(json_data)

###------------------------------------Plot Loss, metric------------------------------------------------

'''
with open('D:\\severance\\UNetr\\metrics.json', 'r') as file:
    json_data = file.read()
...
plt.show()
loss_graph = "D:\\severance\\models\\Unetr\\loss_graph"
...
with torch.no_grad():
    for num in range(20):
        ...
        nib.save(img,f"D:\\severance\\models\\UNetr\\val_result\\{40}-output.nii.gz")
        nib.save(label,f"D:\\severance\\models\\UNetr\\val_result\\{40}-label.nii.gz")
...
with torch.no_grad():
    for num in range(19):
        ...
        nib.save(img,f"D:\\severance\\models\\UNetr\\test_result\\{40}-output.nii.gz")
        nib.save(label,f"D:\\severance\\models\\UNetr\\test_result\\{40}-label.nii.gz")